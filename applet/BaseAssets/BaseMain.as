package BaseAssets
{
	...
	public class BaseMain extends Sprite
	{
		...
		public function varifyLoaderInfo():void{
            var parametros:Object = LoaderInfo(this.root.loaderInfo).parameters;
            var disables:Array = String(parametros['disable']).split(",");
            
			if(disables != null){
				for (var i:int = 0; i < disables.length; i++) 
				{
					if (disables[i] == "RESET_BUTTON") lock(botoes.resetButton);
					if (disables[i] == "TUTORIAL_BUTTON") lock(botoes.tutorialBtn);
				}
			}
        }
